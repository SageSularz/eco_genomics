...
for (i in 1:3) {
  ...
  write.vcf(vcf.thin, 
            file = paste0(full_path, "/projects/eco_genomics/population_genomics/hmk/vcf_final.filtered.thinned_", filter_val, ".vcf.gz"))
  
  # Uncompress the VCF file
  system(paste0("gunzip -c ", full_path, "/projects/eco_genomics/population_genomics/hmk/vcf_final.filtered.thinned_", 
                filter_val, ".vcf.gz > ", full_path, "/vcf_final.filtered.thinned_", filter_val, ".vcf"))
  ...
  geno <- vcf2geno(input.file = paste0(full_path, "/vcf_final.filtered.thinned_", filter_val, ".vcf"),
                   output.file = paste0(full_path, "/projects/eco_genomics/population_genomics/hmk/vcf_final.filtered.thinned_", filter_val, ".geno"))
  
  print(paste("Completed vcf2geno for filter_val =", filter_val))

  #####ITS THE PERIOD IT DOESNT LIKE IT !!!!!
  CentPCA <- LEA::pca("hmk/vcf_final.filtered.thinned_",filter_val,".geno", scale=TRUE)
